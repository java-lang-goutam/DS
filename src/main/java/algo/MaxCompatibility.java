...
public class MaxCompatibility {
    ...
    public int maxCompatibilitySum(int[][] students, int[][] mentors) {
        final boolean[] visited = new boolean[mentors.length];
        helper(students, mentors, visited, 0, 0);
        return max;
    }

    private void helper(int[][] students, int[][] mentors, boolean[] visited, int pos, int score) {

        if (pos == students.length) {
            max = Math.max(score, max); return;
        }

        for (int i=0; i<mentors.length; i++) {
            if (!visited[i]) {
                visited[i] = true;
                helper(students, mentors, visited, pos + 1, score + getCompatibilitySum(students[pos], mentors[i]));
                visited[i] = false;
            }
        }
    }
    ...
    public static void main(String[] args) {
        final MaxCompatibility comp = new MaxCompatibility();
        int[][] students = {{1, 1, 0}, {1, 0, 1}, {0, 0, 1}}, mentors = {{1, 0, 0}, {0, 0, 1}, {1, 1, 0}};
        System.out.println(comp.maxCompatibilitySum(students, mentors));
    }
}
