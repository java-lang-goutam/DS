 ...
 * in meters. It is guaranteed that positioni < positioni+1. speedi is the
 * initial speed of the ith car in meters per second.
...
public class CarsFleetII {
    ...
    public double[] getCollisionTimes(int[][] cars) {
        ...
        for (int i=n-2; i>=0 ; i--) {
            double hitTime = (cars[slowestSoFarPosition][0] - cars[i][0]) / (double)(cars[i][1] - cars[slowestSoFarPosition][1]);

            if (hitTime < 0) {
                result[i] = -1;
                slowestSoFarPosition = i;
                slowestHitTime = Double.MAX_VALUE;
            }

            if (hitTime < slowestHitTime) {
                slowestHitTime = hitTime;
    ...
    public static void main(String[] args) {
        ...
        CarsFleetII cf = new CarsFleetII();
        double[] ct = cf.getCollisionTimes(cars);
