 ...
 * meeting each other at 4. The fleet moves at speed 2. Then, the fleet (speed
 * 2) and the car starting at 4 (speed 1) become one fleet, meeting each other
 * at 6. The fleet moves at speed 1 until it reaches target.
 * Constraints:
 * n == position.length == speed.length 1 <= n <= 105 0 < target <= 106 0 <=
 * position[i] < target All the values of position are unique. 0 < speed[i] <=
 * 106
 */
public class CarsFleet {
    ...
    public int carFleet(int target, int[] position, int[] speed) {
        ...
        final TreeMap<Integer, Double> map = new TreeMap<>(Collections.reverseOrder());

        // we will calculate time required for each car to reach end
        for (int i=0; i<n; i++) {
        ...
        for (Map.Entry<Integer, Double> entry : map.entrySet()) {
            final double timeTakenByCurrentCar = entry.getValue();
            if (maxTime < timeTakenByCurrentCar) {
                maxTime = timeTakenByCurrentCar;
                totalFleet++;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        CarsFleet cf = new CarsFleet();
        int totalFleet = cf.carFleet(target, position, speed);
