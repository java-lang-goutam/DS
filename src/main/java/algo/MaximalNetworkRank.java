...
public class MaximalNetworkRank {
    ...
    public int maximalNetworkRank(int n, int[][] roads) {
        ...
        final boolean[][] connection = new boolean[n][n];
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                int rank = hash[i] + hash[j] + (connection[i][j] ? -1 : 0);
    ...
    public static void main(String[] args) {
        final MaximalNetworkRank rank = new MaximalNetworkRank();
        final int roads[][] = {{0, 1}, {1, 2}, {2, 3}, {2, 4}, {5, 6}, {5, 7}}, n = 8;
        System.out.println(rank.maximalNetworkRank(n, roads));
    }
}
